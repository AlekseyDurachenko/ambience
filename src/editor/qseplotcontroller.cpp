...
QsePlotController::QsePlotController(QObject *parent) :
...
void QsePlotController::setPosition(QseCursor *cursor)
{
    if (cursor != m_position)
        m_position = cursor;
}

void QsePlotController::setSelection(QseSelection *selection)
{
    if (selection != m_selection)
        m_selection = selection;
}

QCursor QsePlotController::defaultCursor() const
{
    return QCursor(Qt::IBeamCursor);
}

void QsePlotController::mouseMoveEvent(QMouseEvent *event, const QRect &rect, const QseGeometry &geometry)
{
...
void QsePlotController::mousePressEvent(QMouseEvent *event, const QRect &rect, const QseGeometry &geometry)
{
    qint64 sample = QseFunc::mapWidgetToSample(event->x(), geometry.x(), geometry.samplePerPixel());
...
void QsePlotController::mouseReleaseEvent(QMouseEvent *event, const QRect &rect, const QseGeometry &geometry)
{
...
void QsePlotController::wheelEvent(QWheelEvent *event, const QRect &rect, const QseGeometry &geometry)
{
...
void QsePlotController::udpateCursor(int x, const QRect &, const QseGeometry &geometry)
{
