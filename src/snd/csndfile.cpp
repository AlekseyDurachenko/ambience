...
sf_count_t CSndFile::qsf_vio_seek(sf_count_t offset, int whence, void *user_data)
{
    sf_count_t pos = 0;
    switch (whence)
    {
...
{
    return reinterpret_cast<QIODevice*>(user_data)->read(reinterpret_cast<char*>(ptr), count);
}

sf_count_t CSndFile::qsf_vio_write(const void *ptr, sf_count_t count, void *user_data)
{
    return reinterpret_cast<QIODevice*>(user_data)->write(reinterpret_cast<const char*>(ptr), count);
}
